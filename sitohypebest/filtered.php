...
if (isset($_SESSION["idutente"]) && isset($_POST["selected"])) {
    ...
    if ($result->num_rows > 0) {
        ...
        $sql = "select post.ID as idpost, username, post.img as imgpost, utente.ID as idutente, descrizione, utente.img as imgutente from post join utente on post.IDUtente = utente.ID where post.situazione like '%$selected%' and pubblicato = 1";
        $result = mysqli_query($conn, $sql);
        if ($result->num_rows > 0) {
